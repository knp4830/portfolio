...
try {
  const { timeline, skills, projects, pages } = await checkContent();
  const chips = skills.groups.reduce((sum, group) => sum + group.items.length, 0);
  console.log(
    `content ok: ${timeline.length} timeline entries, ${skills.groups.length} skill groups (${chips} chips), ${projects.length} projects, site + ${pages.length} page files`,
  );
} catch (error) {
