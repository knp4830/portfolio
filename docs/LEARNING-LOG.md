...
3. **A row of section links on the desk** (`DeskContents.tsx`), beside the day/night switch: Contents · Timeline · Skills · Projects · Contact, as words (Kevin, Sep 22: numbered boxes read as a toolbar, not a notebook; the colophon is one turn past contact and listed on p. 2, so it's left off). Plain `<Link>`s, so a click riffles there like the contents page does and they work without JavaScript; the spread on screen is brighter and underlined from a route-marker CSS rule (right on first paint), and `aria-current="page"` is set alongside the project cards' announcement.
