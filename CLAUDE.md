...
**Wheel, desk contents, corner hints** (same branch): the curl swallows wheel events (non-passive listener + `overscroll-behavior-y: none`), so the page never rubber-bands; `SCROLL_PER_TURN` 600 → 420; numbered contents boxes sit on the desk beside the day/night switch, so any spread can jump to any section; "drag to turn →" (p. 2, ink) and "← drag back" (p. 3) name the gesture. Kevin's new p. 9 line: "I'm in Seattle for now, and I'd love to travel anywhere for work." Verified: 9/9 new, 9/9, 22/22, 25/25, 8/8; 73 unit tests.
